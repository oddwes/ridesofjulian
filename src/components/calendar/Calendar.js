  ...
  const printRows = () => {
    ...
    return (
      [...Array(weekCount).keys()].map(i => {
        ...
        const activitiesForWeek = activities.filter(activity => dayjs(activity.start_date).isBetween(startDate, endDate))
        return (
          <Week startDate={startDate} endDate={endDate} activitiesForWeek={activitiesForWeek} />
        )
      })
    )
  }
