...
import { updateWorkout, deleteWorkout } from '@ridesofjulian/shared';
...
export function WorkoutEditScreen({ workoutId, onClose }: WorkoutEditScreenProps) {
  ...
  const { data: workout, isLoading, error: fetchError } = useWorkout(workoutId);
  ...
  useEffect(() => {
    if (workout) {
      setPageError(null);
      AsyncStorage.getItem(`workout_${workoutId}`).then((savedState) => {
        if (savedState) {
