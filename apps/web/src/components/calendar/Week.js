...
export const Week = ({ startDate, endDate, activitiesForWeek, plannedWorkoutsForWeek = [], gymWorkoutsForWeek = [], onWorkoutClick, onActivityClick }) => {
  const days = [...Array(7).keys()].map((i) => {
    ...
    return <Day activities={activities} plannedWorkouts={plannedWorkouts} gymWorkouts={gymWorkouts} isToday={date.isSame(dayjs(), 'date')} date={date} key={date} onWorkoutClick={onWorkoutClick} onActivityClick={onActivityClick} />;
  })
